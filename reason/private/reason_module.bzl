...
load(":providers.bzl", "ReasonModuleInfo")

load(":reason_compile.bzl", _reason_compile="reason_compile")
...
def _reason_module_impl(ctx):
    ...
    for reason_file in ctx.files.srcs:
        ml_file = ctx.actions.declare_file(
            reason_file.basename.replace(REI_EXT, MLI_EXT)  # for interfaces
...
reason_module = rule(
    attrs={
        "srcs":
        attr.label_list(allow_files=[RE_EXT, REI_EXT], mandatory=True),
        "toolchain":
        attr.label(
            default=
